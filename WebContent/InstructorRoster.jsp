		...
		<table class="table table-bordered table-striped">
			...
			<tbody>
				...
				<c:forEach var="item" items="${classes}">
					...
					<tr>
					<form action="InstructorRoster" method="post" >
						<td>${item.HCourse.name}</td>
						<td>${item.getClassSchedule()}</td>
						<td>${item.getClassroom()}</td>
						<td>${item.HSemester.getSemester()}</td>
						
						<td><input type="hidden" name="classId" value="${item.classId}"></input>
						<input type="submit" class="btn btn-success" value="View Roster"></input></td>
					</form>
					</tr>
				</c:forEach>
			</tbody>
